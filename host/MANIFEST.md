...
**Updated, 2026-09-19 (review follow-up):** now `RequiresMountsFor=/srv/apps /srv/data /srv/library /srv/staging /srv/vault`. `/srv/apps` is Docker's `data-root` (`/var/lib/docker` lives there) and `/srv/data` holds Tier-1 and will hold the Postgres/Immich state; both are on the NVMe and are hard fstab dependencies anyway (no `nofail`), so this is belt-and-braces made explicit rather than a fix for an observed race. Also correcting the paragraph above: `/srv/staging` is *not* on the same disk as `/srv/library`/`/srv/vault` — it is the hynix on the ASM1166 (`/srv/vault` shares the Seagate). It is listed for its own `nofail` reasons. The file name still says "library"; kept to avoid churn. Verified on the 2026-09-19 reboot: `Mounted srv-data/apps/staging/library/vault.mount` all precede `Starting docker.service`, containers see their data, `systemctl --failed` empty. **Known consequence, stated precisely (a first draft of this note said "delay"; that was wrong):** `RequiresMountsFor=` adds `Requires=` *and* `After=` — so if any listed mount **fails**, `docker.service` does **not start at all** ("Dependency failed"), which takes every container down with it. That is intended for the disks whose data the containers need (apps/data/library), because the alternative is silently bind-mounting the empty placeholder directory (the original bug). But `/srv/staging` (the hynix, currently with an interface fault, see `docs/incident-2026-09-18.md`) and `/srv/vault` (no container mounts it) are `nofail` disks by design, and hard-requiring them means a hynix that fails to mount on some boot = full outage. It has mounted on 16 of 16 boots so far, so this is a risk, not an observed event. Left as the review asked pending Ivan's decision; the alternatives are in the incident document.

**Decided, 2026-09-19 (Ivan): drop `/srv/vault`, keep `/srv/staging`.** Live and mirrored file is now `RequiresMountsFor=/srv/apps /srv/data /srv/library /srv/staging`. Reasoning: no container mounts `/srv/vault` (restic runs on the host), so requiring it could only ever cause an outage; `/srv/staging` stays because qBittorrent silently writing torrents into an empty placeholder directory on the root filesystem is a worse failure than Docker refusing to start. Residual risk accepted knowingly: while the hynix has its interface fault (`docs/incident-2026-09-18.md`), a boot where it fails to mount takes Docker down. Mounted on 17 of 17 boots so far.

---

## `etc/ssh/sshd_config.d/10-hardening.conf`

**What it does:** `PubkeyAuthentication yes`, `PasswordAuthentication no`, `KbdInteractiveAuthentication no`, `PermitRootLogin no` — key-only SSH, no root login. This is the real security boundary the passwordless-sudo decision rests on (see the `skit-nopasswd` entry above).

**Why it exists:** set by Ivan in Task 04 §0 at the console before the agent could connect; recorded as a criterion in `docs/report-04.md`.
...
**Updated, 2026-09-22 (Task 13).** `RequiresMountsFor=/srv/apps /srv/data /srv/library`. `/srv/staging` dropped permanently — the hynix it lived on is physically removed and written off (Task 13), so the mount will never exist again; its fstab entry is commented out, not deleted, with a dated note. This happened in two steps during the same incident: first an emergency drop of `/srv/staging` alone (the scripted Task 13 §1.2 fix), which did **not** restore Docker because `/srv/library` (the Seagate) turned out to also be undetected — its native SATA port had failed (found by Ivan on-site: cable/port, not the drive). Emergency second step: `/srv/library` was also dropped from `RequiresMountsFor` so Docker could start at all, with Jellyfin and qBittorrent stopped by hand (both bind-mount `/srv/library`/had bind-mounted `/srv/staging`) to keep them off the empty placeholder anti-pattern this drop-in exists to prevent. Once Ivan moved the Seagate to the ASM1166/AXAGON and it came up clean (0 kernel ata errors), `/srv/library` was put back into `RequiresMountsFor` and Jellyfin/qBittorrent were force-recreated against the real mount. Verified via a full reboot: `journalctl -b` shows `Mounted srv-library.mount` / `Mounted srv-vault.mount` immediately before `Starting docker.service`, both same second; kernel log had **zero** ata errors on any port for the first time since 2026-09-16. Full story: `docs/report-13.md`.
